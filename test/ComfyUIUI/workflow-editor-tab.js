class WorkflowTab {
  ...
  getDisplayableInputCount(nodeId, apiNode) {
   ...
   return Object.entries(apiNode.input.required)
    .filter(([inputName, inputDef]) => {
   ...
   if (config.image_upload === true) {
    return `<div class="input-container">
      ...
      <select id="${id}" data-node-id="${nodeId}" data-input-name="${inputName}">
        ${inputDef[0].map(option => `<option value="${option}" ${value === option ? 'selected' : ''}>${option}</option>`).join('')}
      </select>
      ...
      if (this.workflow[nodeId] && inputName) {
       try {
        ...
        if (uploadResult.success) {
         ...
         const select = this.contentElement.querySelector(`select[data-node-id="${nodeId}"][data-input-name="${inputName}"]`);
         if (select) {
          const exists = Array.from(select.options).some(option => option.value === uploadResult.name);
          if (!exists) {
